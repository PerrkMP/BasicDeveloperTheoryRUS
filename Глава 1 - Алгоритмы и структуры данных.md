...
- **Преимущества:** динамический размер, эффективная вставка и удаление элементов.
- **Недостатки:** доступ к элементам осуществляется последовательно, нет прямого доступа к произвольному элементу.
...
**Стек (Stack)** - это структура данных, работающая по принципу _"Last-In-First-Out"_ (LIFO), что означает, что последний добавленный элемент становится первым удаленным. То есть элементы добавляются и удаляются только с одного конца стека, называемого вершиной _(top)_.

- **Операции:** _push_ (добавление элемента), _pop_ (удаление элемента), _peek_ (просмотр верхнего элемента).
- **Преимущества:** простота реализации, эффективность при работе с ограниченным набором операций.
- **Недостатки:** доступ только к верхнему элементу.

Стек может быть реализован с использованием различных структур данных, включая массивы и связанные списки. Рассмотрим примеры реализации стека на Python, Java и C++:
...
В приведенных примерах стек реализован с использованием встроенных структур данных Python _(список)_, Java _(ArrayList)_ и C++ _(vector)_. Обратите внимание на то, как операции **push**, **pop** и **peek** выполняются с использованием соответствующих методов соответствующих структур данных.
